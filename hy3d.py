...
cache_volume = modal.Volume.from_name("hf-cache-full", create_if_missing=True)
...
image = (
        ...
        .run_commands(
                ...
                "sed -i 's/torch_dtype=torch.float16/torch_dtype=torch.float16, trust_remote_code=True/' Hunyuan3D-2/hy3dgen/texgen/utils/multiview_utils.py",
        )
        .env({
        ...
        .run_commands(
            # バージョン不一致の例外を pass に置換（さっき通ったワンライナー）
            "python3 -c \"import torch.utils.cpp_extension as c; import re; p = c.__file__; s = open(p).read(); open(p, 'w').write(re.sub(r'raise RuntimeError\(CUDA_MISMATCH_MESSAGE.*?\)', 'pass', s))\"",
            # CC/CXX を明示してビルドを実行
            "cd Hunyuan3D-2/hy3dgen/texgen/custom_rasterizer && CC=gcc CXX=g++ CUDAHOSTCXX=g++ python3 setup.py install",
            "cd Hunyuan3D-2/hy3dgen/texgen/differentiable_renderer && CC=gcc CXX=g++ CUDAHOSTCXX=g++ python3 setup.py install",
            )
        .pip_install("pillow")
        )
...
def get_tex_pipeline():
    global _pipeline_tex
    if _pipeline_tex is None:
        from hy3dgen.texgen import Hunyuan3DPaintPipeline
        print("Loading texture pipeline...")
        _pipeline_tex = Hunyuan3DPaintPipeline.from_pretrained('tencent/Hunyuan3D-2') #v2
        #_pipeline_tex = Hunyuan3DPaintPipeline.from_pretrained('tencent/Hunyuan3D-2.1') #2.1
        cache_volume.commit()
    return _pipeline_tex
...
def generate_3d_model(image_bytes: bytes) -> bytes:
    ...
    try:
        ...
        outputs = pipeline(image=input_image,
        ...
        if EN_TEXTURE:
            print("Unloading shape pipeline to free VRAM...")
            del pipeline
            gc.collect()
            torch.cuda.empty_cache()

            pipeline_tex = get_tex_pipeline()
            mesh = pipeline_tex(mesh, image=input_image)
